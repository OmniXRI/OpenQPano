...
void ILI9225_Show_Pano(Mat &imgS, bool bDir)
{
    uint8_t cs_pin[LCD_AMOUNT] =
            {PIN_LCD_CS1,PIN_LCD_CS2,PIN_LCD_CS3,PIN_LCD_CS4,PIN_LCD_CS5,PIN_LCD_CS6};
    uint8_t sx = 0;
    uint8_t sy = 0;
    uint8_t ex = sx + LCD_WIDTH - 1;
    uint8_t ey = sy + LCD_HEIGHT - 1;
    uint8_t *ptrS;

    if(bDir){ // outside to inside
        for(int n=0; n<LCD_AMOUNT; n++){            
            ILI9225_SetXY(cs_pin[n], sx, sy, ex, ey, sx, sy);   // 設定視窗範圍
            ILI9225_CS(cs_pin[n], LOW);       // 晶片選擇線設為低電位
            ILI9225_WR_Cmd(0x22);          // Write Command to GRAM

            for(int y=0; y<=ey; y++){                   // 設定迴圈數為圖框高度
              ptrS = imgS.ptr<uint8_t>(y) + (n * LCD_WIDTH * 2);

              for(int x=0; x<=ex; x++){                 // 設定迴圈數為圖框寬度，每個點為2個位元組(RGB565)
                  ILI9225_WR_Data8(*(ptrS++));  // High Byte寫入LCD GRAM中
                  asm volatile ("nop");
                  ILI9225_WR_Data8(*(ptrS++)); // Low Byte寫入LCD GRAM中
                  asm volatile ("nop");
              }
            }

            ILI9225_CS(cs_pin[n], HIGH);       // 晶片選擇線設為高電位
        }
    }
    else{ // inside to outside, for 360 degree camera
        for(int n=0; n<LCD_AMOUNT; n++){
            ILI9225_SetXY(cs_pin[n], sx, sy, ex, ey, sx, sy);   // 設定視窗範圍
            ILI9225_CS(cs_pin[n], LOW);       // 晶片選擇線設為低電位
            ILI9225_WR_Cmd(0x22);          // Write Command to GRAM
            ...
            for(int y=0; y<=ey; y++){                   // 設定迴圈數為圖框高度
              ptrS = imgS.ptr<uint8_t>(y) + ((LCD_AMOUNT-n) * LCD_WIDTH * 2);

              for(int x=0; x<=ex; x++){                 // 設定迴圈數為圖框寬度，每個點為2個位元組(RGB565)
                  ILI9225_WR_Data8(*(ptrS-2));  // High Byte寫入LCD GRAM中
                  asm volatile ("nop");
                  ILI9225_WR_Data8(*(ptrS-1)); // Low Byte寫入LCD GRAM中
                  asm volatile ("nop");
                  ptrS -= 2;
              }
            }
